...
df_tagged['embeddings'] = df_tagged['embeddings'].apply(lambda x: [float(val) for val in x[1:-1].split()])
...
# Step 2: Flatten the multi-level columns into a single-level
df_tagged_pivot.columns = [f"{col}_{idx}" for col, idx in df_tagged_pivot.columns]

# Step 3: Reset the index to make 'ID' a regular column again
